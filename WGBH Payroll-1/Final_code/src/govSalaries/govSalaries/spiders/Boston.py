...
class BostonSpider(scrapy.Spider):
    ...
    # years = [2017, 2018]
    start_urls = ['govsalaries.com//']
    url = 'https://govsalaries.com/salaries/MA/city-of-boston?year={year}&page={page}'

    def start_requests(self):
        for year in self.years:
            for page in range(1, 168):
                yield scrapy.Request(url=self.url.format(year=year, page=page),
    ...
    def parse_website(self, response):
        ...
        for employee in employees:
            ...
            yield scrapy.Request(url=full_url, meta={'name': name, 'year': year, 'job_title': job_title, 'employer': employer}, callback=self.parse_detail, dont_filter=True)
